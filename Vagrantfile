    ...
    config.vm.define boxname do |box|
        ...
        when "centralRouter"
          box.vm.provision "shell", run: "always", inline: <<-SHELL
            ...
            # add route to office 1 network
            echo "192.168.2.0/24 via 192.168.0.34" >> /etc/sysconfig/network-scripts/route-eth3
            # add route to office 2 network
            echo "192.168.1.0/24 via 192.168.0.35" >> /etc/sysconfig/network-scripts/route-eth3
            sudo systemctl restart network
        SHELL
        ...
        when "office1Router"
           box.vm.provision "shell", run: "always", inline: <<-SHELL
            ...
            # add route to office 2 network
            echo "192.168.1.0/24 via 192.168.0.35" >> /etc/sysconfig/network-scripts/route-eth1
            sudo systemctl restart network
        SHELL
        ...
        when "office2Router"
           box.vm.provision "shell", run: "always", inline: <<-SHELL
            ...
            echo "GATEWAY=192.168.0.33" >> /etc/sysconfig/network-scripts/ifcfg-eth1
            # add route to office 1 network
            echo "192.168.2.0/24 via 192.168.0.34" >> /etc/sysconfig/network-scripts/route-eth1
            sudo systemctl restart network
        SHELL
